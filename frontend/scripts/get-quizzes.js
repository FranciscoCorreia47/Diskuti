...
function renderQuiz(quiz, quizzes_list, comment_section) {
    const name = quiz["name"];
    const creation_date = convertDate(quiz["creation_date"]);
    ...
    comment_icon.addEventListener("click", function(){
        ...
        const comment_close = document.createElement("span");
        ...
        comment_close.classList.add("fa-solid");
        comment_close.classList.add("fa-x");
        comment_close.classList.add("close-comment");

        comment_close.addEventListener("click", function(){
        ...
        comment_header.appendChild(comment_close);
        ...
        for(let comm of Object.values(quiz.comments)){
            ...
            date_span.textContent = `${convertDate(comm.creation_date)}`;
        ...
        if(event.target.classList.contains("fa-magnifying-glass")){
            ...
            for(let quiz of Object.values(data)){
                if(quiz["name"].includes(search_input.value.trim())){
                    renderQuiz(quiz, quizzes_list, comment_section);
